...
| **A skill that writes or spends opens only on user naming, a user request for what it does, a pack skill's name-call, or explicit standing terminology instructions** | the ten explicit descriptions (four core, `session-handoff`, `orchestrate-subagents`, `small-model-handoff`, `fable5-model-routing`, `curate-terminology`, `use-terminology`) | Boundary | Who decides that a procedure runs is the user's call once it lands on disk or on the bill; observed 2026-08-07, the situational descriptions fired across unrelated conversations | **Rises** — a stronger model needs an unrequested procedure less, so the gate costs less and the intrusion costs more |
...
| Separate terminology curation from lookup and application; lookup alone grants no file writes | curate-terminology, use-terminology | Boundary | User-requested split, 2026-09-10; creating a reference and consuming it leave different artifacts | Keep |
...
| Ongoing terminology use and maintenance require explicit project instructions; a later read-only task limits writes | terminology pair | Boundary | User requested always-on compliance, while the pack forbids unrequested procedures | Keep; inspect configured instruction entry points |
