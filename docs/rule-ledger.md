...
| Sharing pass before a document leaves the author's machine: two risk axes, named people see their passages first, removed coordinates handed over separately | write-internal-doc | Boundary | User-set policy 2026-09-05 and 2026-09-06 after two review rounds over nine shared reports; the second round added the personal axis | Keep |
| Absence of a record is stated once as a reading rule and inside label definitions, never as per-sentence qualifiers | write-internal-doc | Verification | User corrections 2026-09-05, nine minutes apart: first the absence rule, then "너무 강한 표현 쓰지마 … 건조한 문서기만 하면 돼" | Keep |
| Claim status is carried by words or table columns, never bracket tags | write-internal-doc | Boundary | User's repository rule (personal AGENTS.md, 2026-09) and the house vocabularies already in use | Keep |
| Every material claim links to a source a colleague can open; shared copies allow only Slack, Notion, GitHub, Linear, and public links | write-internal-doc | Verification | User instructions 2026-09-03 (links on every claim) and 2026-09-05 (ban on local paths in shared copies) | Keep |
| Content decides sections; the design system, not the section template, is shared across a set | write-internal-doc | **Measured workaround** | 2026-09-04: a template the user dictated was applied to every document and rejected the next day; the habit is the model's | **Re-verify per generation; delete when a model no longer templates uniformly** |
