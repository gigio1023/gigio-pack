...
- **Out:** a supplied domain glossary and other general craft. Detailed prose editing, diagram production, and engine-specific conventions belong to their specialist skills. `share-internal-doc` owns the shared document's reader context, supported judgment, depth, language principles, preservation, and recipient suitability across media; specialist tools support that result without dictating a universal outline or format. Pydantic library guidance stays with its official external skill; no third-party Python or modularity pack is bundled.
...
- `share-internal-doc` was authored 2026-09-09 as `write-internal-doc` and renamed 2026-09-10. The September 14–15 revisions separate document principles from medium-specific delivery, remove reader-irrelevant caveats and interface narration, and prioritize explanatory comparisons over dashboard clutter. Static checks, direct feedback, and source rationale are recorded; a new behavioral pilot has not been run.
