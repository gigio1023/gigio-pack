...
Read this when the task changes Pydantic models, validation, or serialization. Pydantic-specific modeling guidance comes from the official external `pydantic` skill; the Python and module-design rules in this package are authored locally.
...
## Local boundary policy

Use Pydantic for selected external validation and serialization boundaries. Internal objects need not become `BaseModel` subclasses; keep a boundary model inside the application when it still represents the same data, without adding a duplicate wrapper automatically. New Pydantic modeling targets v2 when the project supports it. A v1 application needs a scoped migration before adopting incompatible APIs.
...
For containers, unions, `TypedDict`, or dataclasses that need boundary validation without a wrapper model, consult `TypeAdapter` in the official documentation. It is a validation/serialization adapter, not a field annotation.
