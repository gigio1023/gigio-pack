...
- Respect an existing decomposition. When a plan document already assigns dependencies and file ownership — a `.plans/` file written by `gigio-write-plan`, for example — do not re-decompose it; group work by its stage field. Running such a plan end-to-end belongs to `gigio-execute-plan`, which draws on this skill for orchestration mechanics.
