...
- **절차를 시작하는 스킬 9개는 명시적 요청을 요구한다.** 코어 4 + `session-handoff` + `share-internal-doc` + `orchestrate-subagents`/`small-model-handoff`/`fable5-model-routing`의 description은 `Use only when …`으로 연다. 작업 규모, 낯선 도메인, 세션 길이 자체는 실행 요청이 아니다.
...
- **17개**: 코어 4 `gigio-project-setup`/`gigio-write-plan`/`gigio-execute-plan`/ `gigio-review-results` + 발굴 2 `find-unknowns`/`deep-interview` + 실행 지원 5 `orchestrate-subagents`/`small-model-handoff`/`git-worktree-setup`/ `fable5-model-routing`/`python-coding-standards` + 내보내기 4 `commit-and-push`/`draft-pr`/`session-handoff`/`share-internal-doc` + 용어 기록·사용 2 `curate-terminology`/`use-terminology`.
...
- **개명 계보** (agent-skills 원본명 기준): `unknowns-pass`→`find-unknowns`, `handoff-prompt`→`session-handoff`, `lower-capability-executor-prompt`→ `small-model-handoff`(worker-brief 경유 — "worker"가 하네스 내부 서브에이전트로 오독돼 재개명), `fable5-judgment`→`fable5-model-routing`, `parallel-subagents`→`orchestrate-subagents`(무동사→동사-목적어), `write-internal-doc`→`share-internal-doc`(2026-09-10 — "write"는 검색어로 쳤을 때 목적을 연상시키지 못했다. 이 스킬의 목적은 동료에게 공유할 문서이므로 동사를 share로). 유지 4개(deep-interview, git-worktree-setup, commit-and-push, draft-pr)는 원명이 이미 목적을 담고 있음. 두 인계 스킬은 의도적 대구: `session-handoff`(다음 세션에게) ↔ `small-model-handoff`(더 약한 모델에게).
...
The 2026-09-14 `share-internal-doc` revision supersedes its earlier mandatory outline, figure, and Markdown-source prescriptions. Its core owns reader context, supported judgment, depth, language, preservation, recipient safeguards, and the dark default for controlled screen documents and figures. The September 15 refinement also removes the mandatory reading-rule paragraph: keep a qualification where it changes interpretation or action, not to demonstrate caution. Routine definitions, UI tours, self-appraisal, and redundant visual panels need a concrete reader use or should be deleted. Read the detailed references for their stated decisions, not as a compulsory sequence for every edit. Reader questions live in a review reference, not an output template. Document preparation does not imply publication, commits, or installation; use the user's established scope. The collected private transcripts remain outside the public package.

## 현재 상태와 다음 작업

- 코어 4개 저작 완료, 잔류 9개 상호 연결 패스 적용 완료, 이중 리뷰 반영 완료.
- 2026-09-09: `share-internal-doc` 추가(당시 이름 `write-internal-doc`). 사용자 세션 정정 이력에서 규칙을 수집해 저작. 2026-09-10 개명. 같은 날부터 문서와 그림의 색상은 다크가 기본이고 라이트는 사용자가 명시할 때만 별도 렌더로 만든다. 시험 적용 전. 다섯 편의 공유 문서를 만든 뒤 팩 잔류 여부를 재검토한다.
- 2026-09-11: plan 파일을 저장소 코드 밖으로 일반화. 상단 세 절(Goal·Out of scope·Judged against)은 사용자 소유이며 사용자 원문으로 쓰고, 계획자가 제안한 줄은 안내 메시지에서 이름을 밝힌다. `owns`/`avoid`가 `owns-files`/`avoid-files`를 대체해 경로·run 출력 디렉터리·URL/ID로 지정한 외부 리소스를 덮고, `Planned against`가 commit 단일 기준을 대체해 재검증 가능한 기준점 목록이 되며 없는 입력은 대체 없이 차단한다. `find-unknowns`는 저장소 작업만이 아니라 모든 brief를 계획으로 보낸다. `gigio-review-results`는 plan 상단의 사용자 원문을 기준으로 먼저 비교하고, finding마다 해당 줄을 인용하며, 실제 사용한 입력이 Planned against와 같은지 확인하고, 사용자가 확인하지 않은 제안 줄 위에 지어진 작업을 unconfirmed basis로 따로 낸다. 제안 줄은 사용자가 확인하거나 고칠 때까지 Judged against 끝의 한 문장에 남는다. 근거는 실사용 plan 3건(`aim-intelligence/.plans/`)의 형식 파손이며 사용자 피드백 실측은 아직 없다. Authority·Environment·Evidence rules 절 신설은 보류. 한국어 기록은 `docs/design/plan-generalization/`. 다음 plan 3건에서 사용자가 읽고 고친 절을 세어 보류 절의 추가 여부와 기존 절의 삭제 여부를 정한다.
