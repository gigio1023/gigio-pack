...
- 한 요청에 여러 정거장의 작업이 포함되어 있으면 이미 받은 권한을 유지한다.
  각 산출물을 완성한 뒤 요청된 다음 스킬로 진행하며 같은 승인을 다시 묻지 않는다.
  검토·수정·게시·정리의 범위는 구분한다. PR 게시만으로 전역 스킬을 재설치하지 않는다.
...
- **다음: 시험 적용 2건.** 관찰 대상 넷 — 병렬 쓰기 이득, 계획 파일의 워커 간
  인계 충분성, 리드가 명백히 필요 없어 한 단계(빼기 후보) 실측, 그리고 일반
  코드 작업 밖에서 `acceptance` 필드가 실제로 무엇으로 채워지는가.
  **둘 중 하나는 비엔지니어링 성격으로 잡는다** — 실험 케이스(작업 단위가 파일이
  아니라 run, 음성 결과가 산출물)를 추측이 아니라 실측으로 가져오기 위해서다.
